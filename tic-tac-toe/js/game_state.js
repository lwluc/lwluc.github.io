...
const cGameState = function(level) {
  ...
  this.markCell = (row, col) => {
    ...
    if (this.TURN === this.SYMBOL.robot) this.O_MOVES_COUNT++; // This is used for score calculation
  ...
  this.transitionTurn = () => {
    ...
    this.TURN =
      this.TURN === this.SYMBOL.human ? this.SYMBOL.robot : this.SYMBOL.human;
  };
  ...
  this.isGameOver = () => {
    ...
    if (this.GAME_RESULT !== this.RESULTS.incomplete) {
      logger.log(
        'Well, a result is already reached, lets save cpu cycles and do nothing'
      );
      return true;
    }
  ...
  this.getFinishMoves = player => {
    logger.log(
      'Entered into getFinishMoves : ' + player + ' -> ' + this.TOTAL_MOVES
    );
    ...
    do {
      ...
      if (line === player.repeat(2)) {
        if (this.BOARD[0][0] === '') finish_moves.push([0, 0]);
        else if (this.BOARD[1][1] === '') finish_moves.push([1, 1]);
        else if (this.BOARD[2][2] === '') finish_moves.push([2, 2]);
      ...
      if (line === player.repeat(2)) {
        if (this.BOARD[0][2] === '') finish_moves.push([0, 2]);
        else if (this.BOARD[1][1] === '') finish_moves.push([1, 1]);
        else if (this.BOARD[2][0] === '') finish_moves.push([2, 0]);
    ...
    if (finish_moves.length > 0) return finish_moves[0];
  };
};
